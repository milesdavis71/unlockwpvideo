...
class BootstrapView
{
    ...
    protected $boot = null;
    public function __construct(BootstrapRunner $boot)
    {
        $this->boot = $boot;
    }
    ...
    public function renderError()
    {
        ...
        <table class='settings'>
            <?php if ($this->boot->isZip()) { ?>
                <tr>
                    <td>ZipArchive:</td>
                    <td><?php echo $this->boot->hasZipArchive ? '<i class="pass">Enabled</i>' : '<i class="fail">Disabled</i>'; ?> </td>
                </tr>
                <tr>
                    <td>ShellExec&nbsp;Unzip:</td>
                    <td><?php echo $this->boot->shellUnzipEnabled() ? '<i class="pass">Enabled</i>' : '<i class="fail">Disabled</i>'; ?> </td>
                </tr>
            <?php } else { ?>
                <tr>
        ...
        <div style="font-size:11px">
            Please Note: Either ZipArchive or Shell Exec will need to be enabled for the installer to 
...
class SecureCsrf
{
    ...
    public static function removeKeyVal($key, $val)
    {
        self::getCSRFVars();
...
namespace {
    ...
    class InstallerBootstrapData {
        const ARCHIVE_FILENAME       = '20230802_unlock_wp_video_5ed80c578c7a33b46445_20230802070157_archive.zip';
        const ARCHIVE_SIZE           = '49072522';
        const INSTALLER_DIR_NAME     = 'dup-installer';
        const PACKAGE_HASH           = '5ed80c5-02070157';
        const SECONDARY_PACKAGE_HASH = 'b60b1a5-02070157';
        const VERSION                = '4.5.7';
    }
    ...
    $view = '';
    try {
        $boot = new BootstrapRunner(
            InstallerBootstrapData::ARCHIVE_FILENAME,
            InstallerBootstrapData::ARCHIVE_SIZE,
            InstallerBootstrapData::INSTALLER_DIR_NAME,
            InstallerBootstrapData::PACKAGE_HASH,
            InstallerBootstrapData::SECONDARY_PACKAGE_HASH,
            InstallerBootstrapData::VERSION
        );
        LogHandler::initErrorHandler(array($boot, 'log'));
        $bootView = new BootstrapView($boot);
        $view = $boot->run();
    } catch (Exception $e) {
